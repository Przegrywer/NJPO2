...
namespace Battleship.Model
{
    internal class Player
    {
        ...
        private List<Samolot> _myPlanes = new List<Samolot>();
        private List<Samolot> _enemyPlanes = new List<Samolot>();
        private int _shots;
        ...
        public Player()
        {
            ...
            foreach (typSamolotu type in Enum.GetValues(typeof (typSamolotu)))
            {
                _myPlanes.Add(new Samolot(type));
                _enemyPlanes.Add(new Samolot(type));
            }
        ...
        public void Reset()
        {
            ...
            for (int i = 0; i != GRID_SIZE_Y; ++i)
            {
                if (i < 11)
                {
                    for (int j = 0; j != GRID_SIZE_X; ++j)
                    {
                        MyGrid[i][j].Reset(TypPowierzchni.Water, ObiektNaPlanszy.Unknown);
                        EnemyGrid[i][j].Reset(TypPowierzchni.Unknown, ObiektNaPlanszy.Unknown);
                    }
                }
                else
                {
                    for (int j = 0; j != GRID_SIZE_X; ++j)
                    {
                        MyGrid[i][j].Reset(TypPowierzchni.Grass, ObiektNaPlanszy.Unknown);
                        EnemyGrid[i][j].Reset(TypPowierzchni.Unknown, ObiektNaPlanszy.Unknown);
                    }
                }
            }
        ...
        private bool PlaceVerticalShip(int shipIndex, int remainingLength)
        {
            ...
            if (PlacementPossible())
            {
                ...
                {
                    MyGrid[row][startPosCol].Type = TypPowierzchni.Undamaged;
                    MyGrid[row][startPosCol].ObjectType = ObiektNaPlanszy.Ship;
        ...
        private bool PlaceHorizontalShip(int shipIndex, int remainingLength)
        {
            ...
            if (PlacementPossible())
            {
                ...
                {
                    MyGrid[startPosRow][col].Type = TypPowierzchni.Undamaged;
                    MyGrid[startPosRow][col].ObjectType = ObiektNaPlanszy.Ship;
        ...
        private bool PlaceVerticalVehicle(int vehicleIndex, int remainingLength)
        {
            ...
            if (PlacementPossible())
            {
                ...
                {
                    MyGrid[row][startPosCol].Type = TypPowierzchni.Undamaged;
                    MyGrid[row][startPosCol].ObjectType = ObiektNaPlanszy.Vehicle;
        ...
        private bool PlaceHorizontalVehicle(int vehicleIndex, int remainingLength)
        {
            ...
            if (PlacementPossible())
            {
                ...
                {
                    MyGrid[startPosRow][col].Type = TypPowierzchni.Undamaged;
                    MyGrid[startPosRow][col].ObjectType = ObiektNaPlanszy.Vehicle;
        ...
        private bool PlacePlaneUp(int planeIndex, int remainingLength)
        {
            ...
            if (PlacementPossible())
            {
                for (int col = startPosCol; remainingLength != 0; ++col)
                {
                    if (col > startPosCol + 2)
                    {
                        MyGrid[startPosRow + 1][startPosCol + 1].Type = TypPowierzchni.Undamaged;
                        MyGrid[startPosRow + 1][startPosCol + 1].ObjectType = ObiektNaPlanszy.Plane;
                        MyGrid[startPosRow + 1][startPosCol + 1].SquareIndex = planeIndex;
                    }
                    else
                    {
                        MyGrid[startPosRow][col].Type = TypPowierzchni.Undamaged;
                        MyGrid[startPosRow][col].ObjectType = ObiektNaPlanszy.Plane;
        ...
        private bool PlacePlaneRight(int planeIndex, int remainingLength)
        {
            ...
            if (PlacementPossible())
            {
                for (int row = startPosRow; remainingLength != 0; ++row)
                {
                    ...
                    {
                        MyGrid[row][startPosCol].Type = TypPowierzchni.Undamaged;
                        MyGrid[row][startPosCol].ObjectType = ObiektNaPlanszy.Plane;
        ...
        private void PlacePlanes()
        {
            ...
            for (int i = 0; i != _myPlanes.Count && !startAgain; ++i)
            {
                ...
                for (; !placed && loopCounter != 10000; ++loopCounter)
                {
                    int remainingLength = _myPlanes[i].Dlugosc;
        ...
        private void DestroyItem(int i, List<List<SeaSquare>> grid)
        {
            foreach (var row in grid)
            {
                foreach (var square in row)
                {
                    ...
                    {
                        square.Type = TypPowierzchni.Destroyed;
                        square.ObjectType = ObiektNaPlanszy.Destroyed;
                    }
                }
            }
        }
        ...
        public TypPowierzchni FiredAt(int row, int col, out int damagedIndex, out bool isDestroyed)
        {
            isDestroyed = false;
            damagedIndex = -1;
            ...
            switch (MyGrid[row][col].Type)
            {
                ...
                case TypPowierzchni.Undamaged:
                    ...
                    if (square.ObjectType == ObiektNaPlanszy.Ship)
                    {
                        ...
                        {
                            square.Type = TypPowierzchni.Damaged;
                        }
                    }

                    if (square.ObjectType == ObiektNaPlanszy.Vehicle)
                    {
                    ...
                    if (square.ObjectType == ObiektNaPlanszy.Plane)
                    {
                        if (_myPlanes[damagedIndex - 50].StrzelajW())
                        {
        ...
        {
            return _myPlanes.All(plane => plane.czyZniszczony);
        }

        public void ZmienTure(Player otherPlayer)
        {
            ...
            while (!takenShot)
            {
                ...
                if (EnemyGrid[row][col].Type == TypPowierzchni.Unknown &&
                    EnemyGrid[row][col].ObjectType != ObiektNaPlanszy.Destroyed)
                {
